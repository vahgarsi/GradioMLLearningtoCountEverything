...
parser.add_argument("-i", "--input-image", type=str, required=False, help="/Path/to/input/image/file/")
...
def predict_objects(image):
    cv2.imwrite('user_image.png', image)
    segment(image)
    p = pathlib.Path('user_result_out.png')
    return object_counts,p
...
def show_output():
    image_result = cv2.imread("user_result_out.png")
...
def segment(image):
    ...
    if args.bbox_file is None: # if no bounding box file is given, prompt the user for a set of bounding boxes
        ...
        out_bbox_file = "user_test_box.txt"
    ...
    object_counts = output.sum().item()
    rslt_file = "user_result_out.png"
    ...
    print('extracted exemplar features are: ', exemplar_features)
    #image_result = cv2.imread("user_result_out.png")
    #cv2.imshow('result image', image_result)
...
#iface = gr.Interface(fn=predict_objects, inputs="image", outputs=[output_text, output_image])
iface = gr.Interface(fn=predict_objects, inputs="image", outputs=[output_text,output_image])

#interface_image = gr.Interface(fn=predict_image, inputs="", outputs="image")
